...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        output['overall_metrics'] = {'CHAIRs': chair_s,
                                     ...
                                     'F1': f1,
                                     'Len': avg_len,}
        output['hallucinated_indices'] = hallucinated_indices
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cap_file", type=str, default='chair_results',
                        help="path towards json or jsonl saving image ids and their captions in list of dict.")
    parser.add_argument("--image_id_key", type=str, default="id",
    ...
    parser.add_argument("--layer", type=int, default=24, help="Layer index used in steering")
    parser.add_argument("--feature_idxs", type=int, nargs="+", default=[2229], 
                        help="Feature indices used in steering (list)")
    parser.add_argument("--steer_alpha", type=float, default=15.0, help="Steer strength (alpha)")
    parser.add_argument("--substr", type=str, default="lure", help="Substring used in filename (e.g., lure)")

    parser.add_argument("--cache", type=str, default="chair.pkl",
                        help="pre inited CHAIR evaluator object, for fast loading.")
    parser.add_argument("--coco_path", type=str, default='data/coco_annotations',
    ...
    k = len(args.feature_idxs)
    opt_str = "faith"
    cap_file = os.path.join(args.save_dir, f"layer_{args.layer}_{args.steer_alpha}_{k}_{opt_str}_{args.substr}.jsonl")    
    cap_dict = evaluator.compute_chair(cap_file, args.image_id_key, args.caption_key)
    print(f'Evaluating {cap_file}...')
    print_metrics(cap_dict)
    hallucinated_idxs = cap_dict['hallucinated_indices']
    if args.save_path:
        save_hallucinated_words(args.save_path, cap_dict)

    with open("chair.pkl", "rb") as f:
        data = pickle.load(f)
